...
parser = argparse.ArgumentParser(prog="ExtDMN",description="Andre s code ExtDMN for SQUAD data. Use for Q&A, master semester project @EPFL-LIA, 2017")

parser.add_argument('--network', type=str, default="dmn_multiple", choices=['dmn_multiple'], help='network type: dmn_multiple')
...
parser.add_argument('--nbr_ex', type=int, default=1, help='default training and test set are small, exprim in k the size of the exemple to use')
...
if args.mode == 'train':
    print("==> training")
    skipped = 0
    train_f_name =  os.path.join(os.path.dirname(os.path.realpath(__file__)), 'output_data/train_babi%s_metadata_withMem_1.csv' %(args.babi_id))
    test_f_name =  os.path.join(os.path.dirname(os.path.realpath(__file__)), 'output_data/test_babi%s_metadata_withMem_1.csv' %(args.babi_id))
